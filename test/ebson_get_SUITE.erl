...
-easy_group([{group, decoded_values}, {tests, [value_from_empty_list_doc,
					       ...
					       value_from_non_existing_key_in_list_doc]}]).
...
-easy_group([{group, decoded_has_key}, {tests, [has_key_is_false_for_any_key_when_empty_list_doc,
					        has_key_is_true_for_first_key_in_list_doc,
					        has_key_is_true_for_any_existing_key_in_list_doc,
					        has_key_is_false_for_any_non_existing_key_in_list_doc]}]).
...
value_from_non_existing_key_in_list_doc(_) ->
    Key = <<"e">>,
    Doc = [{<<"a">>, 1}, {<<"b">>, 2}, {<<"c">>, 3}],
    undefined = ebson_get:value(Key, Doc).
...
%%%-------------------------------------------------------------------
%%% DECODED HAS KEY TESTS
%%%-------------------------------------------------------------------
has_key_is_false_for_any_key_when_empty_list_doc(_) ->
    Key = <<"a">>,
    false = ebson_get:has_key(Key, []).

has_key_is_true_for_first_key_in_list_doc(_) ->
    Key = <<"a">>,
    Doc = [{Key, 1}],
    true = ebson_get:has_key(Key, Doc).

has_key_is_true_for_any_existing_key_in_list_doc(_) ->
    Key = <<"c">>,
    Doc = [{<<"a">>, 1}, {<<"b">>, 2}, {Key, 3}, {<<"d">>, 4}],
    true = ebson_get:has_key(Key, Doc).

has_key_is_false_for_any_non_existing_key_in_list_doc(_) ->
    Key = <<"e">>,
    Doc = [{<<"a">>, 1}, {<<"b">>, 2}, {<<"c">>, 3}, {<<"d">>, 4}],
    false = ebson_get:has_key(Key, Doc).
